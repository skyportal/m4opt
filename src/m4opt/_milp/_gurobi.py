...
def _flatten_constraints(cts):
    ...
    Comparing a scalar variable yields a single constraint rather than an
    array, so it has to be wrapped before iterating.
    """
    if isinstance(cts, (gp.TempConstr, GurobiEqualityProxy)):
        return (cts,)
...
class GurobiModel:
    ...
    def number_of_variables(self):
        ...
        self._grb.update()
        return self._grb.NumVars

    @property
    def number_of_constraints(self):
        """Number of constraints of every kind in the model."""
        self._grb.update()
        return (
            self._grb.NumConstrs
            + self._grb.NumGenConstrs
            + self._grb.NumQConstrs
            + self._grb.NumSOS
        )
    ...
    def objective_expr(self):
        ...
        self._grb.update()
        return GurobiObjectiveProxy(self._grb.getObjective())
...
class GurobiObjectiveProxy:
    """Wrap a Gurobi objective to expose docplex's term count."""
