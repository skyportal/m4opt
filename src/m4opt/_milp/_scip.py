...
class SCIPModel:
    ...
    def solve(self, **kwargs):
        ...
        solving_time = self._scip.getSolvingTime()

        # An objective limit makes SCIP report plain infeasibility, and it may
        # still hold a solution that the limit rejects. The other backends
        # report this as an abort and return nothing, so match them.
        if self._lowercutoff is not None and (
            status_str == "infeasible"
            or (
                self._scip.getNSols() > 0 and self._scip.getObjVal() < self._lowercutoff
            )
        ):
